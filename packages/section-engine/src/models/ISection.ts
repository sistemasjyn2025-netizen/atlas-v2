...
export class ISection extends StructuralSection {
  constructor(
    id: string,
    code: string,
    name: string,
    ...
    super(id, code, name, SectionShape.I, family, method, metadata);
  }

  public getGeometry(): SectionGeometry {
    // Future implementation: Adapter to Geometry Engine
    return { contours: { outer: { segments: [], isClosed: true }, inner: [] } };
  }

  protected compareDimensions(other: StructuralSection): boolean {
    if (!(other instanceof ISection)) return false;
